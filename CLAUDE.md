...
3. **Stream sync** (`GET /api/launches/[slug]/streams`): `getOrSyncStreams` in `youtubeService.ts` uses an adaptive TTL — 30 min when launch is within ±24h, 12 h otherwise. `?force=true` bypasses the cache. Results are stored in the `StreamSync` collection keyed by `launchId`.
